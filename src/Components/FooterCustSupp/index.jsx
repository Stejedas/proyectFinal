...
function FooterCustSupp() {
  const [t] = useTranslation("global");
  const [theming] = useContext(ThemingContext);

  return (

    <Row className='footer bg-light m-0 p-0' style={{ 'color': `${theming.whiteBlack.color}`, 'background-color': `${theming.blueD.code}` }}>
      <Col xs={3} md={3} xl={2} className='p-0'>
        <div className='d-flex align-items-center justify-content-center h-100 m-0' style={{ 'color': `${theming.whiteBlack.color}`, 'background-color': `${theming.blueN.code}` }}>
          <ThemingSelector props={[theming.blueN.code, theming.whiteBlack.code ]}></ThemingSelector>

        </div>
      </Col>
      <Col xs={6} md={6} xl={8} style={{ 'color': `${theming.whiteBlack.color}`, 'background-color': `${theming.blueN.code}` }}>


        <footer style={{ 'color': `${theming.whiteBlack.color}`, 'background-color': `${theming.blueN.code}` }}>
      ...
      <Col xs={3} md={3} xl={2} className="p-0">
        <Stack className='d-flex align-items-center justify-content-center h-100' style={{ 'color': `${theming.whiteBlack.color}`, 'background-color': `${theming.blueN.code}` }}>
          <MultiLenguage props={[theming.blueN.code, theming.whiteBlack.code ]}></MultiLenguage>
        </Stack>
      </Col>
